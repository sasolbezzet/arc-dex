...
const docs: Record<Lang, { intro: string; warning: DocSection; sections: DocSection[]; agentMcp: { intro: string; sections: DocSection[] } }> = {
  id: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'ARCOX sekarang disiapkan untuk pola Circle Gateway Nanopayments/x402: API premium bisa membalas HTTP 402, buyer menandatangani authorization offchain, lalu settlement batch menjadi roadmap. Fitur ini belum live untuk mengambil pembayaran gas-free.',
        steps: ['Invoice publik tetap pembayaran USDC biasa di Arc Testnet.', 'x402 disabled by default.', 'Tidak ada fee tersembunyi pada invoice merchant.', 'Buka /pay/sandbox untuk cek readiness Nanopayments.'],
      },
      {
        title: 'Agent Jobs',
  ...
  en: {
    ...
    sections: [
      ...
      { title: 'Circle Gateway Nanopayments', body: 'ARCOX is now prepared for the Circle Gateway Nanopayments/x402 pattern: a premium API can return HTTP 402, the buyer signs an offchain authorization, and batch settlement remains roadmap work. Gas-free nanopayments are not live yet.', steps: ['Public invoices remain normal USDC payments on Arc Testnet.', 'x402 is disabled by default.', 'No hidden fee is taken from merchant invoices.', 'Open /pay/sandbox to check Nanopayments readiness.'] },
  ...
  zh: {
    ...
    sections: [
      ...
      { title: 'Circle Gateway Nanopayments', body: 'ARCOX 已为 Circle Gateway Nanopayments/x402 模式做准备：premium API 可以返回 HTTP 402，buyer 进行 offchain authorization 签名，batch settlement 仍是 roadmap。gas-free nanopayments 还不是 live。', steps: ['Public invoice 仍是 Arc Testnet 上的普通 USDC payment。', 'x402 默认关闭。', '不会从 merchant invoice 中隐藏收费。', '打开 /pay/sandbox 查看 Nanopayments readiness。'] },
